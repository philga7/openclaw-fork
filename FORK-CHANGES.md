...
### Native reasoning streaming and typing indicators

- **Handle native `thinking_*` events in the agent subscribe handler**
  - OpenAI-compatible providers that stream reasoning via `reasoning_content` (GLM-5, DeepSeek-R1, Ollama reasoning models) emit `thinking_start`/`thinking_delta`/`thinking_end` events through `pi-ai` → `pi-agent-core`. These were silently dropped by `handleMessageUpdate` in `src/agents/pi-embedded-subscribe.handlers.messages.ts` because the early return only allowed `text_delta`/`text_start`/`text_end` through.
  - **Fix:** The handler now recognizes `thinking_start`, `thinking_delta`, and `thinking_end` event types. On `thinking_delta`, it extracts the accumulated thinking content and calls `emitReasoningStream()` (when `reasoningMode: "stream"`), which flows through `onReasoningStream` → `signalReasoningDelta()` → `typing.refreshTypingTtl()`, keeping the typing/thinking indicator alive indefinitely during long model thinking chains. On `thinking_end`, `onReasoningEnd` is called for parity with the tag-based path.
  - **Typing indicator persistence:** Previously, the typing indicator expired after the 2-minute TTL during silent model inference because no events refreshed it. Now every native reasoning token resets the TTL, so the indicator stays active for the full duration of the model's thinking phase — critical for long-running reasoning models like GLM-5 in the cloud.
  - **Streaming reasoning now works for native providers:** `reasoningLevel: "stream"` previously only worked for tag-based providers (Google Gemini, etc.) that embed `<think>` tags in the text stream. Native reasoning providers now get real-time reasoning streaming via the `"thinking"` agent event stream and the `onReasoningStream` callback.
  - Native thinking events are also logged to the raw stream as `assistant_thinking_stream` for debugging.
