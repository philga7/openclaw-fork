...
- **Ollama**
  ...
  - **GLM-5 malformed tool call resilience** — GLM-5 (and potentially other Ollama-hosted models) can emit malformed tool calls (bad JSON arguments, null function names, empty `tool_calls` arrays) which, when re-injected into session history, cause Ollama Cloud to return HTTP 400. Three-layer fix:
    1. **Source sanitization** (`src/agents/ollama-stream.ts`): `buildAssistantMessage` now validates each tool call before adding it to content — checks for non-null `function` object, non-empty `name`, and coerces `arguments` to a valid object (parses JSON strings, rejects non-objects). Malformed calls are dropped with `console.warn`. The `stopReason` is derived from the filtered content so an all-malformed response becomes `"stop"` not `"toolUse"`. Errors thrown on `!response.ok` now carry a `.status` property so `resolveFailoverReasonFromError` can classify them by HTTP status code.
    2. **Session replay sanitization** (`src/agents/session-transcript-repair.ts`): `hasToolCallInput` strengthened from a null check to full validation — requires `input`/`arguments` to be a plain object; string values are only accepted if they parse to a valid JSON object. `repairToolCallInputs` logs WARN for each dropped tool call and for entire assistant messages dropped when all tool calls were malformed.
    3. **Error classification for fallback** (`src/agents/pi-embedded-helpers/errors.ts`): Added format error patterns (`/ollama api error 400/i`, `/invalid tool call/i`, `/malformed.*tool/i`) so Ollama 400s are classified as `"format"` and routed through the existing Gemini fallback chain even without a `.status` property (defense in depth).
    - Tests: `src/agents/ollama-stream.test.ts` (5 new), `src/agents/session-transcript-repair.e2e.test.ts` (4 new), `src/agents/failover-error.e2e.test.ts` (4 new).
    - **Tool schema audit** — Schemas most likely to trigger hallucination-driven malformed calls: `VoiceCallToolSchema` (extensions/voice-call, 6-variant `Type.Union`), all Feishu schemas (doc/wiki/perm/drive, top-level `Type.Union` with nested enum unions), `CronToolSchema` (`additionalProperties: true` on `job`/`patch`), `MessageToolSchema` (3 levels of nested Discord component schemas). Core tool schemas (`BrowserTool`, `GatewayTool`, `NodesTool`, etc.) already use flat `Type.Object` with `stringEnum` and are low risk.
...
- **Hooks loader (`pathToFileURL` import)** — The internal hooks loader (`src/hooks/loader.ts`) uses `pathToFileURL` from `node:url` to build file URLs for dynamic `import()` of directory-discovered handlers. The import was previously misplaced inside the file’s JSDoc comment block, so at runtime the directory-based hook path threw `ReferenceError: pathToFileURL is not defined` when loading the first hook. Fix: move `import { pathToFileURL } from "node:url"` out of the comment into the real import block. Path validation (`isPathInsideWithRealpath`) is unchanged and still runs before any URL is built; no security impact. Directory-based hooks (including Foundry and managed/workspace hooks) now load correctly.
...
- **foundry_write_extension null checks**
  - Vendored `foundry-openclaw` extension into `extensions/foundry-openclaw` with fix for tool template generation in `writeExtension` ("Cannot read properties of undefined (reading STR)" when property `type` or `description` was missing). Added fallbacks: `v.type || "string"`, `v.description || k`, and `t.description || t.name`. Plugin-sdk exports `OpenClawPluginToolContext` and `OpenClawPluginToolFactory` for extension compatibility.
